...
| :--- | :--- |
| **Shift**（修飾キー） | — |
| **Clip 前** 前のイベントへ／Shift で先頭へ | `ReplaySelectPreviousEvent`（通常）、`ReplaySelectFirstEvent`（Shift） |
| **Clip 後** 次のイベントへ／Shift で末尾へ | `ReplaySelectNextEvent`（通常）、`ReplaySelectLastEvent`（Shift） |
| **UserScript1～3** ユーザー定義スクリプト（[4.6.1](#section-4-6-1)） | `ScriptStart`（`Value=Iryx_RP00_UserScript1`～`3`（通常）、`5`～`7`（Shift）） |
| **Channel A～D** チャンネル選択 | `ReplaySelectChannelA`～`ReplaySelectChannelD` |
| **UserScript4～5**（[4.6.1](#section-4-6-1)） | `ScriptStart`（通常は `Value=Iryx_RP00_UserScript4`～`5`、Shift 時は 4→8、5→5） |
| **Jump In** 選択中の In 点へ | `ReplayJumpToSelectedInPoint` |
| **Jump Out** 選択中の Out 点へ | `ReplayJumpToSelectedOutPoint` |
| **Play** 再生 | `ReplayPlay`（通常）／Shift 時は TCP 送信なし |
| **Pause** 一時停止 | `ReplayPause` |
| **Play Out** リプレイの起動・再生準備（[詳細](#section-4-6-play-out)） | 通常: `ReplayPlayAllEvents`→`ReplayPause`／Shift: `ReplayPlayAllEvents` のみ |
| **Rec** 録画開始／停止 | `ReplayStartRecording`（通常）、`ReplayStopRecording`（Shift） |
| **Angle 1～6** カメラアングル | `ReplayCamera1`～`ReplayCamera6`（通常）、`ReplaySelectedEventSingleCameraOn`（値1～6）（Shift） |
| **Angle 7～8** カメラアングル | `ReplayCamera7`～`ReplayCamera8`（通常）、`ReplaySelectedEventSingleCameraOn`（値7～8）（Shift） |
| **Mark In** In 点の設定／更新 | `ReplayMarkIn`（通常）、`ReplayUpdateSelectedInPoint`（Shift） |
| **Mark Out** Out 点の設定／更新 | `ReplayMarkOut`（通常）、`ReplayUpdateSelectedOutPoint`（Shift） |
| **Jump To Now** 現在時間へジャンプ（[詳細](#section-4-6-jump-to-now)） | 通常: `ReplayJumpToNow`→`ReplayJumpFrames`→`ReplayPlay`／Shift: `ReplayJumpToNow` のみ |
| **Quick Clip** ワンボタンクリップ（[詳細](#section-4-6-quick-clip)） | 通常: `ReplayMarkInOut`／Shift: 押下で `ReplayMarkIn`、離しで `ReplayMarkOut` |
| **Copy** コピー／複製 | `ReplayCopySelectedEvent`（通常）、`ReplayDuplicateSelectedEvent`（Shift） |
| **Delete** 削除 | 送信なし（通常）、`ReplayDeleteSelectedEvent`（Shift） |
| **Menu** メニュー表示 | — |
| **Back** メニュー操作（戻る） | — |
| **Enter** メニュー操作（決定） | — |
...
- Shortcuts または Scripting の設定で、名前を `Iryx_RP00_UserScript1` ～ `Iryx_RP00_UserScript8` としたユーザー定義スクリプトを用意し、実行したい処理を登録してください。

**物理キーと N の対応**

- 専用キーは UserScript1～5 の 5 つです。
- `Shift` を押しながら UserScript1～4 を押すと、それぞれ `Iryx_RP00_UserScript5`～`Iryx_RP00_UserScript8` に対応する起動になります。
